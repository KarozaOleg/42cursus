...
/*   Updated: 2020/11/25 22:45:04 by mgaston          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	cast_var->yintercept = floor(game->player->y /
game->map->scaled_to) * game->map->scaled_to;
	cast_var->yintercept += game->ray->is_ray_facing_down ?
game->map->scaled_to : 0;
	cast_var->xintercept = game->player->x +
(cast_var->yintercept - game->player->y) / tan(game->ray->angle);
	...
	cast_var->xstep *= (game->ray->is_ray_facing_left &&
cast_var->xstep > 0) ? -1 : 1;
	cast_var->xstep *= (game->ray->is_ray_facing_right &&
cast_var->xstep < 0) ? -1 : 1;
}

void	return_casting_step_horisontal(t_ray_cast_var *cast_var, t_ray *ray,
